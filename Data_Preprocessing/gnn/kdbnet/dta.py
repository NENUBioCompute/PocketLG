...
class DTATask(object):
    ...
    def pdb_graph_db(self):
        if self._pdb_graph_db is None:
            self._pdb_graph_db = pdb_graph.pdb_to_graphs(self.prot2pdb, self.prot_featurize_params)
    ...
    def build_data(self, df, onthefly=False):
        ...
        if onthefly:
            prot_featurize_fn = partial(pdb_graph.featurize_protein_graph, **self.prot_featurize_params)
...
class DAVIS(DTATask):
    ...
    def __init__(self,
        ...
        prot_pdb_id = json.load(open(pdb_json, 'r'))
