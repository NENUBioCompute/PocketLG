...
def pdb_to_graphs(prot_data, params):
    ...
    #print('图表示：', graphs)
    # for pdb_name, graph in graphs.items():
        # print(f"Protein: {pdb_name}")
        # print("Sequence:", graph.seq)
        # print("\n")
    return graphs
...
def featurize_protein_graph(
    ...
    with torch.no_grad():
        ...
        #esm_list = ast.literal_eval(protein['ESM'])
        #seq_emb = torch.tensor(esm_list, dtype=torch.float32)          #esm_seq的开关
...
def _local_frame(X, edge_index, eps=1e-6):
    ...
    O = torch.stack((o_1, n_2, torch.cross(o_1, n_2, dim=-1)), 1)
